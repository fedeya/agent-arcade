...
- Run only the plugin package check with `bun run --filter @agent-arcade/opencode-plugin typecheck`.
- There are no configured test, lint, format, build, or CI workflows in this repo right now; do not invent those commands.
- The only workspace package is `packages/opencode-plugin`, named `@agent-arcade/opencode-plugin`.
