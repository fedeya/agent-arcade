...
- Run only the plugin package check with `bun run --filter opencode-agent-arcade typecheck`.
- There are no configured test, lint, format, build, or CI workflows in this repo right now; do not invent those commands.
- The only workspace package is `packages/opencode-plugin`, named `opencode-agent-arcade`.
- Plugin entrypoint is `packages/opencode-plugin/src/tui.tsx`; `.opencode/tui.json` loads that TSX source directly for local OpenCode TUI usage.
...
- The runner game is wired as `tui.tsx -> games/runner/index.ts -> RunnerOverlay -> RunnerGame`; OpenCode agent/session events become in-game floaters through `arcade/feed.ts`.
